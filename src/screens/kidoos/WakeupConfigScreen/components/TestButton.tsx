...
import { useTestDreamWakeup } from '@/hooks';
...
export function TestButton({
  ...
  const testWakeup = useTestDreamWakeup();
  ...
  const handlePress = useCallback(() => {
    if (isTestActive) {
      ...
      // Démarrer le test
      const currentColor = color || '#FFC864';
  ...
  }, [isTestActive, kidooId, color, brightness, testWakeup]);
  ...
  }, [color, brightness, isTestActive, kidooId, testWakeup]);
  ...
  return (
    <View style={styles.container}>
      <Button
        title={
          isTestActive
            ? t('kidoos.dream.wakeup.stopTest', { defaultValue: 'Arrêter le test' })
            : t('kidoos.dream.wakeup.test', { defaultValue: 'Tester' })
        }
